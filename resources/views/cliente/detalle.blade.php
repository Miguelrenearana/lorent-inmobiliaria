...
@section('contenido')
<div style="width: 100%; max-width: 100vw; padding: 15px;" class="mx-auto flex flex-col md:flex-row gap-5 items-start">
    <div class="card flex-1 w-full">
        <div class="w-full relative overflow-hidden bg-slate-100 mb-5 rounded-lg">
            @if($propiedad->imagen)
                <img src="{{ asset('storage/' . $propiedad->imagen) }}" alt="{{ $propiedad->titulo }}" class="w-full h-auto object-cover rounded-lg">
            @else
